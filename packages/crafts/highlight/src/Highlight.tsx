...
export const Highlight = ({
	...
	const [editorSettingsOpen, setEditorSettingsOpen] = useState(isSettingsOpen);
	const [editorLayout, setEditorLayout] =
		useState<EditorLayout>(defaultEditorLayout);
	...
	useEffect(() => {
		setOpen(isOpen);
		// setEditorSettingsOpen(isOpen);
	}, [isOpen]);

	useEffect(() => {
		setEditorSettingsOpen(isSettingsOpen);
	}, [isSettingsOpen]);
	...
	const handleMenuKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
		...
		if (
			...
			document.activeElement === editorSettingsRef.current
		) {
							...
							>
								{group.map((item) => {
									...
									return (
										<button
											aria-checked={
												item.type === "toggle" ? checked : undefined
											}
											...
											role={
												item.type === "toggle" ? "menuitemcheckbox" : "menuitem"
											}
											type="button"
										>
											<Icon
